...
const MUITable = ({ elements = [] }: Props) => {
  ...
  const handleDeleteConfirm = async (element_id) => {
      ...
      try {
        const res = await fetch(StrapiUrl+"rules-hours/" + element_id, {
          method: "PUT",
          headers: {
    ...
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Typography variant='h5'>
          Reglas por Rango de Horas
        </Typography>
      </Grid>
      <Grid item xs={12}>
        <Grid>
          <Button component='a' variant='contained' sx={{ px: 5.5 }} onClick={() => push("/rules-hours/create")}>
            Nuevo Registro
          </Button>
        </Grid>
                  ...
                  .map((row) => {
                    return (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>{row.places_name}</TableCell>

                        <TableCell>{row.type_rule}</TableCell>
                        ...
                        <TableCell>
                          <Button component='a' size='small' variant='contained' style={{background: '#17a2b8', color: 'white'}} onClick={() => push("/rules-hours/edit/"+row.id)}>
                            Editar
                          </Button>
                          &nbsp;
...
export const getServerSideProps = async (context) => {
  ...
  const res = await fetch(StrapiUrl+"rules-hours?populate=%2A&filters[deleted][$not]=true&filters[site][id]="+site_id_logueado+"&sort[0]=order%3Aasc");
  ...
  try {
      ...
      for (let i = 0; i < response.data.length; i++) {
        ...
        try{
          for (let i2 = 0; i2 < row.attributes.places.data.length; i2++) {

            const row2 =       row.attributes.places.data[i2];

            places_name += row2.attributes.name+' | ';
        ...
        elements.push(
            {
              ...
              type_rule:  row.attributes.type_rule,
