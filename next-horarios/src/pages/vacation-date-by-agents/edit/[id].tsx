...
const FormLayouts = ({ agents = [], data }: Props) => {
  ...
  const updateElement = async (area: ElementData) =>
    await fetch(StrapiUrl+"vacation-date-by-agents/"+element_id, {
      method: "PUT",
      headers: {
      ...
      <Grid container spacing={6}>
        ...
        <Grid item xs={12}>
          <Typography variant='h5'>
            Actualizar Vacaciones por Agente
          </Typography>
        </Grid>
...
export const getServerSideProps = async (context) => {
  ...
  const res2 = await fetch(StrapiUrl+"vacation-date-by-agents/"+id+"?populate=%2A");
  ...
  let agent_value = 0
  let days_before_to_remind_value = 0;
  ...
  try {
    ...
    days_before_to_remind_value = response2.data.attributes.days_before_to_remind;
