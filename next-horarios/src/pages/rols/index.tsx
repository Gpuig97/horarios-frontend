...
const MUITable = ({ elements = [] }: Props) => {
  ...
  return (
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Grid>
          <Button component='a' variant='contained' sx={{ px: 5.5 }} onClick={() => push("/rols/create")}>
            Nuevo Registro
          </Button>
        </Grid>
                  ...
                  .map((row) => {
                    return (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>
                          <Button component='a' size='small' variant='contained' style={{background: '#17a2b8', color: 'white'}} onClick={() => push("/rols/edit/"+row.id)}>
                            Editar
                          </Button>
                          &nbsp;
...
export const getServerSideProps = async (context) => {
  const res = await fetch(StrapiUrl+"rols?populate=%2A&filters[deleted][$not]=true");
