...
const MUITable = ({ elements = [] }: Props) => {
  ...
  const handleDeleteConfirm = async (element_id) => {
      ...
      try {
        const res = await fetch(StrapiUrl+"configuracion-horas-nocturnas/" + element_id, {
          method: "PUT",
          headers: {
    ...
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Typography variant='h5'>
          Configuración Horas Nocturnas
        </Typography>
      </Grid>
      <Grid item xs={12}>
        <Grid>
          <Button component='a' variant='contained' sx={{ px: 5.5 }} onClick={() => push("/configuracion-horas-nocturnas/create")}>
            Nuevo Registro
          </Button>
        </Grid>
              ...
              <TableBody>
                {elements
                  ...
                  .map((row) => {
                    return (
                      <TableRow key={row.id}>
                        <TableCell>{row.hora_desde}</TableCell>
                        <TableCell>{row.hora_hasta}</TableCell>
                        <TableCell>{retornarFechaLegible(row.createdAt)}</TableCell>
                        <TableCell>
                          <Button component='a' size='small' variant='contained' style={{background: '#17a2b8', color: 'white'}} onClick={() => push("/configuracion-horas-nocturnas/edit/"+row.id)}>
                            Editar
                          </Button>
                          &nbsp;
...
export const getServerSideProps = async (context) => {
  ...
  const res = await fetch(StrapiUrl+"configuracion-horas-nocturnas?populate=%2A&filters[deleted][$not]=true");
  ...
  try {
      ...
      for (let i = 0; i < response.data.length; i++) {
        ...
        elements.push(
            {
              id: row.id,
              hora_desde: row.attributes.hora_desde,
              hora_hasta: row.attributes.hora_hasta,
