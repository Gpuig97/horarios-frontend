...
const MUITable = ({ elements = [] }: Props) => {
  ...
  const handleDeleteConfirm = async (element_id) => {
      ...
      try {
        const res = await fetch(StrapiUrl+"tipo-standbies/" + element_id, {
          method: "PUT",
          headers: {
  ...
  return (
    <Grid container spacing={6}>
      ...
      <Grid item xs={12}>
        <Grid>
          <Button component='a' variant='contained' sx={{ px: 5.5 }} onClick={() => push("/tipo-standbies/create")}>
            Nuevo Registro
          </Button>
        </Grid>
                  ...
                  .map((row) => {
                    return (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>
                          <Button component='a' size='small' variant='contained' style={{background: '#17a2b8', color: 'white'}} onClick={() => push("/tipo-standbies/edit/"+row.id)}>
                            Editar
                          </Button>
                          &nbsp;
...
export const getServerSideProps = async (context) => {
  ...
  const res = await fetch(StrapiUrl+"tipo-standbies?populate=%2A&filters[deleted][$not]=true&filters[site][id]="+site_id_logueado);
