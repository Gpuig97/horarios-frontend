...
const FormLayouts = ({ agents = [], data }: Props) => {
  ...
  const updateElement = async (area: ElementData) =>
    await fetch(StrapiUrl+"dates-available-by-agents/"+element_id, {
      method: "PUT",
      headers: {
      ...
      <Grid container spacing={6}>
        ...
        <Grid item xs={12}>
          <Typography variant='h5'>
            Actualizar Días Disponibles por Agente
          </Typography>
        </Grid>
...
export const getServerSideProps = async (context) => {
  ...
  const res2 = await fetch(StrapiUrl+"dates-available-by-agents/"+id+"?populate=%2A");
  ...
  try {
    ...
    agent_value= response2.data.attributes.agent.data.id;
